...
**Bounded context** — A region of the domain with its own ubiquitous language and its own rules; the same word can mean different things in different contexts.
...
**Content page** — A wiki page that is not a structural file; the unit of editorial work in use-cases/02-review-wiki-quality, use-cases/03-revise-wiki, and use-cases/04-sync-wiki-with-source-changes.

**Correction assignment** — The input to a corrector: page path, finding, recommendation, and source reference; structurally compatible between Wiki Revision and Drift Detection, enabling corrector reuse.
...
**Exploration report** — A structured summary of one facet of the source code (API surface, architecture, configuration, etc.), produced by a researcher and consumed by the developmental editor (use-cases/01-populate-new-wiki) or proofreaders (use-cases/02-review-wiki-quality).
...
**Internal event** — A coordination event consumed only within a single use case (e.g., IssueIdentified, DriftDetected); not cataloged in the events catalog.
...
**Local fallback** — Markdown files written to `workspace/artifacts/{owner}/{repo}/reports/review-fallback/{date-time}/` when GitHub is unreachable during use-cases/02-review-wiki-quality; preserves findings outside both clones.
...
**Orchestrator** — Abstract parent for the four coordinating actors in editorial use cases; each child has a distinct drive — commissioning (use-cases/01-populate-new-wiki), oversight (use-cases/02-review-wiki-quality), fulfillment (use-cases/03-revise-wiki), alignment (use-cases/04-sync-wiki-with-source-changes) — but shares the same mechanics: workspace resolution, editorial context absorption, dispatch, result collection, and summary presentation.

**Commissioning orchestrator** — The orchestrator in use-cases/01-populate-new-wiki; drive is commissioning; named for the *commissioning editor* who identifies what content is needed, recruits authors, and shepherds manuscripts to completion.

**Oversight orchestrator** — The orchestrator in use-cases/02-review-wiki-quality; drive is oversight; named for the *managing editor* who runs the editorial quality process and ensures standards are met.

**Fulfillment orchestrator** — The orchestrator in use-cases/03-revise-wiki; drive is fulfillment; named for the *production editor* who manages the flow of corrections through the production pipeline.

**Alignment orchestrator** — The orchestrator in use-cases/04-sync-wiki-with-source-changes; drive is alignment; named for the *revisions editor* who manages the process of updating existing content to reflect new information.
...
**Proofread cache** — Ephemeral storage (`workspace/artifacts/{owner}/{repo}/.proofread/`) for coordinating actors during a single use-cases/02-review-wiki-quality review run; created at start, cleaned up at end.
...
**Published language** — The integration contract between bounded contexts; in this system, the GitHub issue body conforming to `documentation-issue.md` is the published language between Editorial Review and Wiki Revision.
...
**Safety check** — Inspection of the wiki working tree for uncommitted changes and unpushed commits before decommissioning (use-cases/06-decommission-workspace); the gate that prevents silent destruction of unpublished work.
...
**Targeted edit** — A surgical change to a specific section of a wiki page, preserving surrounding content; the required editing style for correctors in use-cases/03-revise-wiki and use-cases/04-sync-wiki-with-source-changes.
...
**Type-to-confirm** — The confirmation pattern for destructive actions: the user types the repository name to confirm deletion when unsaved changes exist (use-cases/06-decommission-workspace); replaces `--force` flags.

**Ubiquitous language** — Terms that have precise meaning within a single bounded context; defined in each context file.
...
**Wiki plan** — A hierarchical structure of sections containing pages, each with filename, title, description, and key source files; proposed by the developmental editor, refined and approved by the user before writing begins (use-cases/01-populate-new-wiki).
...
**Workspace selection** — The shared protocol every use case (except use-cases/05-provision-workspace) executes to resolve which workspace to operate on; implemented by `resolve-workspace.sh`.

**Writing assignment** — The input to a creator in use-cases/01-populate-new-wiki: page file path, title, description, key source files, audience, tone, and editorial guidance.
